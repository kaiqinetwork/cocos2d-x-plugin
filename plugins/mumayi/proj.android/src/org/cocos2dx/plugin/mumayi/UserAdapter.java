...
public class UserAdapter implements InterfaceUser {
	private static final String LOG_TAG = "mumayi.UserAdapter";
			...
			public void run() {
				if (SDKWrapper.getInstance().isInited()) {
					SDKWrapper.getInstance().userLogout(mActivity, new ILoginCallback() {
						@Override
						public void onSuccessed(int code, String msg) {
							actionResult(UserWrapper.ACTION_RET_LOGOUT_SUCCESS, msg);
						}
						@Override
						public void onFailed(int code, String msg) {
							actionResult(UserWrapper.ACTION_RET_LOGOUT_FAIL, msg);
						}
					});
				}else {
	            	actionResult(UserWrapper.ACTION_RET_LOGIN_FAIL, "initSDK fail!");
	...
	public void setUserInfo(Hashtable<String, String> devInfo) {
		if (!devInfo.isEmpty()) {
			...
			if (nickName != null) {
				PaymentCenterInstance.getInstance(mActivity).setUserArea("1"); 
				PaymentCenterInstance.getInstance(mActivity).setUserName(nickName); 
				PaymentCenterInstance.getInstance(mActivity).setUserLevel(1); 
			}
		}
	}
